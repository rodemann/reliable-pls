library(dplyr)
source("R/standard_supervised_pen.R")

set.seed(34059349)
method = "standard_supervised_pen"
...
for (iter in 1:N) {
  ...
  #train/test split
  n_test = nrow(data_frame)/2 %>% round()
  ...
  results_list <- standard_supervised_pen(labeled_data = labeled_data, 
  ...
  newx = test_data[,!(names(test_data) %in% c(target))] %>% as.matrix

  scores = predict(model, newx = newx, type = "response") 
...
path = paste(getwd(),"/results/standard_supervised_pen_",
