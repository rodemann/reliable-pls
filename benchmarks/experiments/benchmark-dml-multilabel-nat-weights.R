library(dplyr)
source("R/diff_marg_likelihood_all_ext.R")

set.seed(3405934)
method = "diff_marg_likelihood_all_ext_nat_weights"
...
for (iter in 1:N) {
  ...
  results_list <- diff_marg_likelihood_all_ext(labeled_data = labeled_data, 
                                               unlabeled_data = unlabeled_data,
                                               target = target,
                                               glm_formula = formula,
                                               crit = "nat-weighted sum")
...
path = paste(getwd(),"/results/diff_marg_likelihood_all_ext_nat_weights",
